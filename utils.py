...
def linear_regression(dates, sl_prices, sl_test_date, sl_df, forcastingDays):
    ...
    X_forecast = X[-forecast_out:]  # set X_forecast equal to last given input
    X = X[:-forecast_out]  # remove last given input from X
...
def SVR_rbf(dates, prices, test_date, sl_df, forcastingDays):
    ...
    df = sl_df[['close']]
    forecast_out = int(forcastingDays)  # predicting forcastingDays days into future
    df['Prediction'] = df[['close']].shift(-forecast_out)
...
def random_forests(dates, sl_prices, sl_test_date, sl_df, forcastingDays):
    ...
    X_forecast = X[-forecast_out:]  # set X_forecast equal to last forcastingDays
    X = X[:-forecast_out]  # remove last forcastingDays from X
